...
function EndowmentFields({ form, set }) {
  return (
    <>
...
export default function PolicyCreate() {
  const [form, setForm]       = useState({ pol_type: 'H' });
  ...
  async function submit(e) {
    ...
    try {
      const data = await api.createPolicy(form);
      setResult(data);
    } catch (err) {
      ...
      <form onSubmit={submit}>
        <div className="form-row">
          <div className="field"><label>Customer Number *</label>
            <input value={form.cust_num || ''} onChange={set('cust_num')} required /></div>
          <div className="field"><label>Policy Type *</label>
            <select value={type} onChange={set('pol_type')}>
        ...
        <button className="btn btn-primary" type="submit" disabled={loading}>
          {loading ? <span className="spinner" /> : 'Create Policy'}
        </button>
      </form>
